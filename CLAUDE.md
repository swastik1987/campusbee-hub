...
| 016 | `016_categories_rls_and_seed.sql` | Fixes `class_categories` returning empty for all users: drops stale SELECT policies, creates `categories_public_select` policy (SELECT where `is_active=true OR is_active IS NULL`), seeds 8 top-level categories idempotently (Sports, Martial Arts, Dance, Arts & Craft, Academics, Music, Fitness & Yoga, Languages) with icon names matching the frontend `CATEGORY_ICONS` map, GRANTs SELECT to `anon` + `authenticated`. |
